...
const ScrollToTop = () => {
  ...
  useEffect(() => void window.scrollTo({
    top: 0,
    behavior: "smooth"
  }), [pathname]);
  return null;
...
const IndustryCard = ({
  title,
  icon
}: Industry) => <motion.div variants={item} whileHover={{
  y: -6
}} whileTap={{
  scale: 0.985
}} transition={{
...
const IndustriesPage = () => {
  return <div className="min-h-screen flex flex-col bg-[#F8F9FB]">
      <ScrollToTop />

      <main className="flex-grow pt-1 md:pt-0">
        {/* Hero Section */}
        <section className="relative overflow-hidden bg-gradient-to-r from-gray-900 to-brand-navy text-white">
          ...
          <div className="relative z-10 container mx-auto px-4 py-10 md:py-14 bg-brand-navy">
            <motion.div initial={{
            opacity: 0,
            y: 18
          }} animate={{
            ...
            duration: 0.5
              ...
              <p className="text-white/90 text-base md:text-lg">
                Tailored logistics & infrastructure for high-performance operations
                across key sectors.
              </p>
            </motion.div>
          </div>
